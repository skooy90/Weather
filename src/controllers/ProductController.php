<?php
class ProductController {
    private $productModel;

    public function __construct($productModel = null) {
        $this->productModel = $productModel ?? new Product();
    }

    public function handleRequest() {
        try {
            $method = $_SERVER['REQUEST_METHOD'];
            $path = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
            $id = null;

            // URL에서 상품 ID 추출
            if (preg_match('/\/api\/products\/([^\/]+)/', $path, $matches)) {
                $id = $matches[1];
            }

            switch ($method) {
                case 'GET':
                    if ($id) {
                        $this->getProduct($id);
                    } else {
                        $this->getProducts();
                    }
                    break;
                case 'POST':
                    $this->createProduct();
                    break;
                case 'PUT':
                    if ($id) {
                        $this->updateProduct($id);
                    } else {
                        $this->sendResponse(400, ['error' => '상품 ID가 필요합니다']);
                    }
                    break;
                case 'DELETE':
                    if ($id) {
                        $this->deleteProduct($id);
                    } else {
                        $this->sendResponse(400, ['error' => '상품 ID가 필요합니다']);
                    }
                    break;
                default:
                    $this->sendResponse(405, ['error' => '허용되지 않은 메소드입니다']);
                    break;
            }
        } catch (Exception $e) {
            $this->sendResponse(500, ['error' => $e->getMessage()]);
        }
    }

    private function getProduct($id) {
        try {
            $product = $this->productModel->getProduct($id);
            $this->sendResponse(200, $product);
        } catch (Exception $e) {
            $this->sendResponse(404, ['error' => $e->getMessage()]);
        }
    }

    private function getProducts() {
        try {
            $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
            $limit = isset($_GET['limit']) ? (int)$_GET['limit'] : 10;
            $products = $this->productModel->getProducts($page, $limit);
            $this->sendResponse(200, $products);
        } catch (Exception $e) {
            $this->sendResponse(500, ['error' => $e->getMessage()]);
        }
    }

    private function createProduct() {
        try {
            $data = json_decode(file_get_contents('php://input'), true);
            if (!$data) {
                throw new Exception('잘못된 요청 데이터입니다');
            }
            $id = $this->productModel->createProduct($data);
            $this->sendResponse(201, ['id' => (string)$id]);
        } catch (Exception $e) {
            $this->sendResponse(400, ['error' => $e->getMessage()]);
        }
    }

    private function updateProduct($id) {
        try {
            $data = json_decode(file_get_contents('php://input'), true);
            if (!$data) {
                throw new Exception('잘못된 요청 데이터입니다');
            }
            $this->productModel->updateProduct($id, $data);
            $this->sendResponse(200, ['message' => '상품이 성공적으로 수정되었습니다']);
        } catch (Exception $e) {
            $this->sendResponse(404, ['error' => $e->getMessage()]);
        }
    }

    private function deleteProduct($id) {
        try {
            $this->productModel->deleteProduct($id);
            $this->sendResponse(200, ['message' => '상품이 성공적으로 삭제되었습니다']);
        } catch (Exception $e) {
            $this->sendResponse(404, ['error' => $e->getMessage()]);
        }
    }

    private function sendResponse($statusCode, $data) {
        http_response_code($statusCode);
        header('Content-Type: application/json');
        echo json_encode($data);
        exit;
    }
}
?> 